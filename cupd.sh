...
for d in anaconda miniconda anaconda3 miniconda3
 ...
 if [[ -d $hd/$d ]]; then
  cdir=$hd/$d/$sd
  $cdir/conda install --yes matplotlib scipy numpy astropy pandas xlrd h5py scikit-image bokeh python conda ipython ephem pip numba spyder ipython-notebook paramiko basemap #mkl
